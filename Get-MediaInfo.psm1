...
REVISIONS
convert-BinaryToDecimalStorageUnits 
* *3.7.4 # 4:32 PM 9/1/2025 updated: convert-BinaryToDecimalStorageUnits() to latest: coerce all instances of the $size to double, it's defaulting to string on inbound string
...
function Get-MediaInfoRAW
{
    ...
    *3.7 7/31/21 - stax76's posted rev of orignal Get-MediaInfoSummary() lines
    ...
    Switch to create a matching XML metadata export file (with -Path name/location and .xml ext).[-ExportToFile]
    .EXAMPLE
    ...
    .EXAMPLE
    $files|%{"==$($_)" ; if(test-path $_){ get-mediainforaw -path $_ -exporttofile | out-null } } ; 
    Process an array  of video file paths, and generate output exported -media.xml files for each, (while suppressing echos/returns into out-null). 
    .LINK
    https://github.com/stax76/Get-MediaInfo
    .LINK
    https://github.com/tostka/Get-MediaInfo
